...
/// besides its main `ram_ra`/`ram_inc`, in committed program mode it also
/// stages the `Val_init` program-image contribution. That staged opening is
/// folded into `RamValCheckOutputClaims`, so the aggregate is genuinely
/// one-field-per-instance. But the stage-4 Fiat-Shamir append order
/// interleaves it around the register openings — the program image comes
/// *before* the register openings, then `ram_ra`/`ram_inc` come *after* —
/// which a plain per-instance concatenation cannot express. The stage
/// therefore opts out of the generated absorb methods via
...
/// (`ram_ra`/`ram_inc`) with the staged program-image opening (see its
/// `wire_output_openings` override), so the generated output-shape
/// count/validator cover its presence and count.
...
impl<F: JoltField> Stage4OutputClaims<F> {
    ...
    /// prover's commitment (flush) order exactly: the committed program-image
    /// contribution, the register read-write openings, then the RAM value-check
    /// `ram_ra`/`ram_inc` openings. The program-image opening is produced by the
    /// RAM value-check instance but is *appended first* (before the registers),
    /// so this is hand-written rather than a per-instance concatenation — see
    /// [`Stage4Sumchecks`].
...
pub struct Stage4ClearOutput<F: JoltField> {
    ...
    /// `output_values` for the register and RAM value-check leaves. The
...
pub struct Stage4ZkOutput<F: JoltField, C> {
    ...
    /// `output_points`. Read through the same `*_point()` accessors. The
...
mod tests {
    ...
    /// staged program-image opening, the order is the five register openings
    /// then the two RAM value-check openings. A wrong order here silently
    /// breaks soundness, so it is pinned with distinct sentinels.
    #[test]
    fn opening_values_follow_canonical_order_without_program_image() {
    ...
    /// The full interleaved order: the program-image contribution comes
    /// *first*, then the five register openings, then `ram_ra`/`ram_inc` last —
    /// exactly matching the prover's stage-4 `pending_claims` flush order.
    #[test]
    fn opening_values_interleave_program_image_then_registers_then_ram() {
        let claims = Stage4OutputClaims::<Fr> {
            registers_read_write: registers_claims(),
            ram_val_check: RamValCheckOutputClaims {
